...
// The functions. 
// The _searchFunc( search_param, comparison_param )
...
// The _searchArray( search_param, obj, _token )
    ...
    switch( _token ){
      ...
      case "":
        ...
        var _matched = [];
          ...
          _searchFunc( search_param, obj[i].tagName ) === null ? null : _matched.push( obj[i] );
...
// The Array prototypes.
// The getJsonById( search_param, obj, _token )
	Array.prototype.getJsonById = function( search_param ){ return _searchArray( search_param, this, "#" ); };




// The getJsonByClassName( search_param, obj, _token )
	Array.prototype.getJsonByClassName = function( search_param ){ return _searchArray( search_param, this, "." ); };




// The getJsonByName( search_param, obj, _token )
	Array.prototype.getJsonByName = function( search_param ){ return _searchArray( search_param, this, "_" ); };




// The getJsonByTagName( search_param, obj, _token )
	Array.prototype.getJsonByTagName = function( search_param ){ return _searchArray( search_param, this, "" ); };
...
var editor = 
{
  ...
  events:
    ...
    load:
      {
      writer: function(_param)
        {

        },
/* Loads resources for different platforms */
      styles: function(_is_compatible_)
        {
        var _xtag_ = xtag.tags[editor.focus.nodeName.toLowerCase()],
            _module_ = _xtag_["modules"],
            _platform = base.platform,
            _default = "";

        for(var key in _is_compatible_)
          {

          if(_is_compatible_[key]==="auto")
            {
            /* Auto generates a stylesheet dependent on platform window object properties. */
            /* Information regarding the stylesheet will updated in the xtag.tags[modules]. */
            }
          else if(/url(?=\()/.test(_is_compatible_[key])===true)
            {
            /* Allows you to link or bring in stylesheets via ajax or lin. */
            }
          else if(_is_compatible_[key]==="default")
            {
            /* The default key word indicates to use this properties styles. */

            editor.style.platform = key;

            _default = key;

            }
          else if(typeof _is_compatible_[key] === "object")
            {
            /* Uses x-style json to generat the appropriate style sheets */
            }
          else
            {
...
/* style property contains the default values for x-editor _elements */
  style:
    {
/* type property accepts "inline" or "css" */
    type: null,
    platform: "isDesktop",
    sheet:
      {
...
/* Updates editor component information such as the focus, viewport, and container data. */
    var _return = false;
      ...
      if(_jsn.component !== null)
      {

      _return = true;

      this.viewport = _jsn.component.viewport._getDOM();
      this.container = _jsn.component.container._getDOM();

      this.bin[_element.id] = _jsn;

      _jsn.component.sheet["id"] = _element.id;
      this.sheet = _jsn.component.sheet;

      _jsn.focus === true ?
        (
        this.focus = _element,
        this.style.type = _jsn.component.style.type
        )
        :
        (
        this.focus = null
      ...
      else if(/url(?=\()/g.test(_jsn.component))
        {

        // AJAX code for JSON or HTML-JSON retrieval

        }
      else if(_jsn.component==="default"||!_jsn.component)
        {
...
var base = 
{
  ...
  xtag: function()
    { return base.json.getJsonByTagName("x-base")[0]||null; },
  attributes: 
    {
    autoId: 
      {
      bin: [],
      length: function(){ return this.attributes.autoId.bin.length }
      },
    autoClass: 
      {
      bin: [],
      length: function(){ return this.attributes.autoClass.bin.length }
      }
    },
  ...
  style:
    ...
    sheet: function(_jsn)
      ...
      for(var _key in _jsn.style)
        {

        if(typeof _jsn.style[_key]==="object")
          {

          if(_key==="container")
            {
            var _container_sheet_ = "";

            _container = _jsn.container + "{" + this.write(_jsn.style[_key]) + "}"; 

            _style_sheet_ += _container;

            }
    
          if(_key==="viewport")
            {
            _viewport = _jsn.viewport + "{" + this.write(_jsn.style[_key]) + "}"; 

            _style_sheet_ += _viewport;

            }

          if(_key==="line")
            {
            _line = "x-line{" + this.write(_jsn.style[_key]) + "}"; 
          ...
          if(_key==="textCursor")
            {
            var _blink = _jsn.style[_key].blink||"1s",
                _width = _jsn.style[_key].width||"5px",
                _height = _jsn.style[_key].height||"16px;";

            var _keyframe = "@keyframes textCursorBlink{0%{opacity:1;}100%{opacity: .15;}}",
                _style = "#auto-textCurosr{animation-name:textCursorBlink;animation-duration:" + _blink + ";animation-iteration-count:infinite;animation-play-state:running;" + _width + ";height:" + _height + "}";

            _textCursor = _keyframe + _style;

            _style_sheet_ += _textCursor;

            }

          if(_key==="title")
            {
            _title = "x-title{" + this.write(_jsn.style[_key]) + "}";
          ...
          if(_key==="text")
            {
            _text = "x-text{" + this.write(_jsn.style[_key]) + "}";
        ...
        else if(typeof _jsn.style[_key]==="string" && _key !== "type")
          {
          ...
          if(/[A-Z]/.test(_key))
            ...
            _sheet_ += _key.replace(/[A-Z]/, function(string){ return "-" + string.toLowerCase() } ) + ":" + _jsn.style[_key] + ";";

            }
          else
            {

            _sheet_ += _key + ":" + _jsn.style[_key] + ";";
      ...
      _style_sheet_ += "#" + editor.focus.id + "{" + _sheet_ + "}";

      return _style_sheet_;

      },

    createStyle: function(_element,_platform,_jsn,_type)
      {

      _type === undefined ?
        (
        _jsn.style.type === undefined ?
          _type = "css" : _type = _jsn.style.type
        )
        :
        (
        _type=_type
      ...
      // Check to see if the type property is set to css or inline.
      if(_type==="css")
        {
        // Creates a stylesheet using a style tag and set attributes
        var _style = document.createElement("style");

        _style.setAttribute("name",_element.id+"style");
        _style.setAttribute("type","text/css");
        _style.innerHTML = this.sheet(_jsn);
        ...
        // needs to be finished next for styling.
    ...
    platform: function(event)
      {
      /* Returns an object with information regarding the users platform and updates base.platform. */

      var _platform = base.platform;

      _platform.screen.width = window.screen.width;
      _platform.screen.height = window.screen.height;
      _platform.screen.availWidth = window.screen.availWidth;
      _platform.screen.availHeight = window.screen.availHeight;
      _platform.screen.ratio = window.screen.height/window.screen.width;
      _platform.screen.orientation = window.screen.orientation;

      _platform.name = navigator.platform;
      _platform.codeName = navigator.appCodeName;
      _platform.agent = navigator.userAgent;
      _platform.fullScreen = window.fullScreen;

      if(window.ontouchstart){ _platform.touch = true; }
      else{ _platform.touch = false; }

      if (window.DeviceOrientationEvent){ _platform.oreintationEvent = true; }
      else {  _platform.oreintationEvent = false; }

      if (window.DeviceMotionEvent){ _platform.motionEvent = true; }
      else {  _platform.oreintationEvent = false; }

      if (window.DeviceMotionEvent){ _platform.motionEvent = true; }
      else {  _platform.oreintationEvent = false; }

      if (navigator.vibrate){ _platform.vibrate = true; }
      else {  _platform.vibrate = false; }
    ...
    write: function(_jsn)
      ...
      for(var _key in _jsn)
        ...
        if(/[A-Z]/.test(_key))
          ...
          _style_sheet_ += _key.replace(/[A-Z]/, function(string){ return "-" + string.toLowerCase() } ) + ":" + _jsn[_key] + ";";

          }
        else
          {

          _style_sheet_ += _key + ":" + _jsn[_key] + ";";
  ...
  _getDOM: String.prototype._getDOM = function()
    {
    var _token = this[0];
    ...
    switch(_token)
      ...
      case ".":
      return document.getElementsByClassName(this.replace(/\./, ""));
      case "_":
      return document.getElementsByName(this.replace(/\_/, ""));
      default:
	...
	_toJSON: String.prototype._toJSON = function(_array) 
  	{

  /* Step 1: replace empty string values marked up with open or closed single qoutes with  closed double qoutes. */
    var _emptyStringValue = this.replace( /\'\'(?=\,)|\'\s+\'(?=\,)|\'\'(?=\})|\'\'\s+\'\'(?=\,)|\'\'\s+\'\'(?=\})/g, '""' ),
  
  /* Step 2:  Replaces non-qouted property names with qouted property names.*/	
  	_fixLiteralNotation = _emptyStringValue.replace(/\w+(?=\:)/g, function( string ){ return '"' + string + '"' } ),
  	
  /* Step 3:  Remove unneeded white-space. */
  	_space = _fixLiteralNotation.replace( /\s+(?=\"\w+\"\:)|\s+(?=\[)|\s+(?=\{)|\s+(?=\])|\s+(?=\})/g, "" ),
  	
  /* Step 4:  replaces line breaks with a space */
  	_lineBreak = _space.replace( /\n+| +/g, " " );
  
  /* step 5 */
  var _fixStringTypeA = _lineBreak.replace(/\'/g, '"' ), _fixStringType = _fixStringTypeA.replace(/\"\"/g, '""'),
  ...
  var _jsonObject = "";
  	...
  	_array === false ?
  	  ...
  	  _array === undefined ?
          ...
          (
  	      _jsonObject = "please check your parameter, allowed params: true, false, not set"
  	      )
  	    );
...
// Notes: x-json x-tag definition
var xBase = xtag.register( "x-base", 
    {
    methods:
      {
      json: function()
        {
        var _status = this.getAttribute("json") || this.getAttribute("data-json");

        if(base.json.getJsonById(this.id) === undefined)
          {

          return _status._toJSON(true);

          }
        else
          {

          return base.json.getJsonById(this.id);
      ...
      getJson:
        function()
          ...
          if(this.hasAttribute("json")===true||this.hasAttribute("data-json")===true)
            {
            ...
            this.hasAttribute("id") === true ? 
              _id = this.id
              :
              _id = this.addAutoId(this.nodeName.toLowerCase()+"___autoIdCreation");

            this.hasAttribute("class") === true ? 
              _class = this.className
              :
              _class = this.addAutoClass(this.nodeName.toLowerCase()+"___autoClassCreation");

            this.setAttribute("json",'{"id":"' + _id + '","tagName":"' + this.nodeName.toLowerCase() + '","class":"' + _class + '"}');
      ...
      addAutoId:
        function(_string_pattern_)
          {

          _string_pattern_ = '{"id":"' + _string_pattern_ + "___" + base.attributes.autoId.bin.length + '___"}';
          _string_pattern_ = _string_pattern_._toJSON(false);

          base.attributes.autoId.bin.push(_string_pattern_);

          this.id = _string_pattern_.id;
      ...
      addAutoClass:
        function(_string_pattern_)
          ...
          _string_pattern_ = '{"class":"' + _string_pattern_ + "___" + base.attributes.autoClass.bin.length + '___"}';
          _string_pattern_ = _string_pattern_._toJSON(false);

          base.attributes.autoClass.bin.push(_string_pattern_);

          this.className = _string_pattern_.class;
    ...
    lifecycle: 
      ...
      created: 
        function()
          {
/* Creates a new json for the x-tag element. */

            var _jsn = this.getJson(),
                _nodeName = "";

            /x(?=\-)/.test(this.nodeName.toLowerCase()) === true ?
              _nodeName = this.nodeName.toLowerCase()
              :
              _nodeName = "x-" + this.nodeName.toLowerCase();

            if(xtag.tags[_nodeName].length === undefined)
              {

              xtag.tags[_nodeName].length = 0;

              }

            var _stringy = "";

            this.nodeName.toLowerCase() === "x-base" ?
              (
              document.getElementsByTagName("x-base").length === 1 ?
                (
                xtag.tags[_nodeName].base = [],
                xtag.tags[_nodeName].base.push(_jsn),
                xtag.base.push(_jsn)
                ) 
                :
                (
                "x-base element not present or more than one is being used. Tip: Only one x-base element is allowed."
                )
              )
              :
              (
              /* check for the modules property. */
              xtag.tags[_nodeName].modules === undefined ? 
                xtag.tags[_nodeName].modules = "{}"._toJSON(false)
                :
                "",
              base.xtag().modules.getJsonById(this.id) !== undefined ?
                (
                xtag.tags[_nodeName].modules[xtag.tags[_nodeName].length] = base.xtag().modules.getJsonById(this.id),
                base.json.getJsonById(this.id).component = base.xtag().modules.getJsonById(this.id).component,
                xtag.tags[_nodeName].length += 1
                )
                : 
                (
                _stringy = xtag.tags[_nodeName].modules,
                _jsn.component = null,
                _stringy[xtag.tags[_nodeName].length] = _jsn,
                base.xtag().modules.push(_jsn),
                xtag.tags[_nodeName].length += 1
                )
              );
        ...
        inserted: function()
          {
          /* Checks the browser for settings and sets the platform property on insertion. */
          base.style.platform();
...
/* xtag registration for x-editor */
  xtag.register( "x-editor", 
    {
    prototype: xBase.prototype,
    methods:
      {
      createEditor: function()
        {
        var _focus = editor.focus,
            _jsn = base.json.getJsonById(editor.focus.id),
            _component = _jsn.component;

        var _viewable_lines_ = _component.viewableLines,
            _viewed_lines_ = _component.viewedLines,
            _sheet_ = _component.sheet;

        for(var i=1;i<=_viewed_lines_.length;i++)
          {
          var _line = document.createElement("x-line");

          _line.innerHTML = _sheet_[_viewed_lines_[i-1].toString()];

          editor.focus.appendChild(_line);
    ...
    lifecycle:
      ...
      created:
        function()
          {
/* Find than update the editor object */
          var _jsn = base.xtag().modules.getJsonById(this.id), 

          _isEditor = editor.update(this,_jsn);

          // if editor.update returns true. It means a component object existed for this element.
          if(_isEditor===true)
            {
            var _events = _jsn.component.events;

            // Check component event object for styles property
            if(_events.load.styles)
              {
              var _style_event_ = editor.events.load.styles(_jsn.component.events.load.styles);

              base.style.createStyle(this,_style_event_,_jsn.component);

              }
            else
              {

              base.style.createStyle(this,_style_event_,_jsn.component.style,_jsn.component.style.type);

              }

            // Check component event object for writer property
            if(_jsn.component.events.load.writer)
              {

              editor.events.load.writer(_jsn.component.events.load.writer);
            ...
            base.style.createStyle(this,_jsn.component.style,_jsn.component.style);
...
/* xtag registration for x-line */
  xtag.register( "x-line",
    {
    lifecycle:
      {
      inserted: function()
        ...
        this.innerHTML = "<x-title></x-title>";
