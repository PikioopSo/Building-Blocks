...
/* Retrieve json from the json-base element. */
          var _data = base.getDomJson(this),
              _nodeName = this.hasAttribute("is") === true ? this.getAttribute("is") : this.nodeName.toLowerCase();

/* Create a new modules array else push json to modules array. */
          if(xtag.tags[_nodeName].modules === undefined && typeof _data === "object") 
            { xtag.tags[_nodeName].modules = []; xtag.tags[_nodeName].modules.push(_data); }
          else if(xtag.tags[_nodeName].modules === undefined && typeof _data === "array")
            { xtag.tags[_nodeName].modules.push(_data); }

/* Create a new modules array else push json to modules array. */
          for(var c=0;c < _data.length; c++)
            {

            if(/json(?=\-)/g.test(_data[c].nodeName)===true)
              {
              base.tags[this.nodeName.toLowerCase()] = _data[c]
              }
            else
              {
              base.tags["json-" + _data[c].nodeName.toLowerCase()] = _data[c];
              }
...
/* [json-form]&[json-form events] */
/* [json-form creation event] */
var formCreation = xtag.addEvents("x-tag",
  {
...
var xForm = xtag.register( "json-form",
  ...
  content: function()
    {/*
    <section class="editor-auto-tools">
      <fieldset class="scroll-switch">
        <input type="button" class="form-switch"/>
      </fieldset>
      <fieldset class="tool-menu">
        <input type="button" class="drop-button" is="json-menu"/>
      </fieldset>
    </section>
    */},
  methods:
    {
    json: function(jsonElement)
      {
      return base.tags[jsonElement].modules.getJsonById(this.id);
      }
    },
  lifecycle:
    {
...
/* [x-editer] */
var xEditor =  xtag.register( "x-editor", 
    {
    methods:
      {
      contents: function(jsonElement)
        {
        var _focus = editor.focus,
            _jsn = this.json(jsonElement);
        ...
        for(var i in _sheet_)
          ...
          _title.innerHTML = i;
          _line.innerHTML = _sheet_[i];

          _focus.appendChild(_line);
          _line.appendChild(_title);

          }

          _focus.parentNode.appendChild(_preferences);
...
/* Find than update the editor object */
          ...
          var _jsn = this.json("json-form"),
            ...
            _jsn.focus===true ? 
              document.body.appendChild(base.style.createStyle(this,_jsn.style,_jsn)) : editor.style.bin.push(base.style.createStyle(this,_jsn.style,_jsn));
