...
/* String Prototype */
  ...
  String.prototype._toJSON = function(_array,_string)
    ...
    try
      {
    	_array === true ?
    	  ( _jsonObject = JSON.parse(_fixFinalComma), base.json.push(_jsonObject) ) : 
    	  _array === false ? 
...
var keyVault = 
  ...
  jsonBase:
    ...
    class:'memory',
    required:true,
    style:'position:asbolute;top:0px;left:0px;width:0%;height:0px;opacity:0;overflow:hidden;'
    },
  'json-header':
    {
    class:'header-element', title:'Welcome',
    style:
      {
      style:'position:asbolute;top:0px;left:0px;width:100%;height:50px;padding:0px;margin:0px;opacity:1;overflow:hidden;display:block;'
      },
    nav:
      {
      class:'header-navigation', title:'Site Navigation',
      style:'position:asbolute;top:0px;left:0px;width:200px;height:50px;padding:0px;margin:0px;opacity:1;overflow:hidden;display:inline-block;background-color:rgb(221,221,255)'
      },
    tool:
      {
      class:'header-toolbar', title:'Site Toolbar',
      style:'position:asbolute;top:0px;left:200px;width:75px;height:50px;padding:0px;margin:0px;opacity:1;overflow:hidden;display:inline-block;background-color:rgb(255,255,255)'
      }
    },
  'json-canvas':
    {
    class:'media-canvas', title:'Please enjoy',
    style:
      {
      style:'position:relative;width:auto;height:auto;padding:3px;margin:0px;opacity:1;overflow:auto;display:inline-block;background-color:rgb(221,221,186)'
      },
    tool:
      {
      class:'canvas-toolbar', title:'Media Toolbar',
      style:'position:asbolute;top:0px;left:0px;width:100%;height:50px;padding:0px;margin:0px;opacity:1;overflow:hidden;display:inline-block;background-color:rgb(255,255,255)'
      }
    },
  'json-form':
    {
  ...
  'json-footer':
    ...
    style:
      ...
      opacity:"1",overflow:"visible", display:"block"
      }
    }
  };
...
var base =  
  {
	...
	jScheme:
	  {
	  theseKeyLocks: ['preference','tool','mode','nav','viewport','add','remove', 'scroll', 'toggle','editor','anchor','link','meta','img','checkbox','selection'],
	  id: [],
	  class: [],
	  name: [],
	  is: [],
	  title: [],
	  sheet: [],
	  addCube: function(jsn)
	    {
	    /* addCube() is for creating new xml element bindings for json container elements */
	    },
	  addKeyLock: function(jsn)
	    {
	    /* addKeyLock() is for creating properties for the json container elements */
	    },
    checkKeyLock: function(key)
      {
      for(var i=0; i < this.theseKeyLocks.length; i++)
        {
        var _return = key === this.theseKeyLocks[i] ? true : false;
  ...
  methods:
    ...
    render: function()
      ...
      if(document.getElementsByName(this.id+"_style").length === 0)
        { document.body.appendChild(base.style.createStyle(this,_style,jsn) ); }
      },
    getJson: function()
      { return xtag.tags["json-base"].modules.getJsonById(this.id); }
    }
  };
...
var schemeMixin = xtag.mixins.schemeMixin = 
  ...
  methods:
    ...
    jScheme: function(isElem,_jsn)
      {
      var _jsn = _jsn||this.getJson(),
      isElem = isElem||this;

        _jsn.id ? isElem.id =_jsn.id : (_jsn.id = "default" + base.jScheme.id.length, isElem.id = _jsn.id),
        _jsn.class ? isElem.className = _jsn.class : (_jsn.class = "default" + base.jScheme.class.length, isElem.className = _jsn.class),
        _jsn.name ? isElem.name = _jsn.name : (_jsn.name = "default" + base.jScheme.name.length, isElem.name = _jsn.name),
        _jsn.title ? isElem.setAttribute("title", _jsn.title) : (_jsn.title = "default" + base.jScheme.title.length, isElem.setAttribute("title", _jsn.title))

      base.jScheme.id.push(_jsn.id);
      base.jScheme.class.push(_jsn.class);
      base.jScheme.name.push(_jsn.name);
      base.jScheme.title.push(_jsn.title);

      _jsn.is ? true : _jsn.is = isElem.nodeName.toLowerCase();
    ...
    writeSvgFragment: function(svgFragment,jsonSvgElement)
      ...
      _style.innerHTML ="#" + jsonSvgElement.id + "{" + base.style.write(jsonSvgElement.style) + "}";
  ...
  /* [elements] creates new xml following json-tag rules. */
    elements: function(_data)
      {
      // Iterate through the object and find the objects, strings, nulls, and other data types and property names 
      for(var k in _data)
        {
        var jsn = _data[k],
            el = jsn.for !== undefined ? jsn.for._getDOM() : this;

        if(typeOf(jsn) === "object" && /style/gi.test(k) !== true && k !== "sheet")
          {
          if(base.jScheme.checkKeyLock(k)===false)
            {
            var _doc = jsn.is === "svg" ?
                ( _doc = xtag.createFragment('<svg></svg>').childNodes[0], this.writeSvgFragment(_doc,jsn) ) :
                document.createElement(jsn.is);

            this.jScheme(_doc, jsn);
            xtag.tags["json-base"].modules.push(jsn);
            el.appendChild(_doc);
            }
          else
            {
            this.jScheme(xtag.queryChildren(el, jsn.is)[0], jsn);
            xtag.tags["json-base"].modules.push(jsn);
            el.appendChild(xtag.queryChildren(el, jsn.is)[0]);
            }
          }
        else if(typeOf(jsn) === "object" && k === "sheet")
          {
          var _tar = jsn.for._getDOM()||el,
              _doc = this.jSheet(_tar,jsn);
          
          _tar !==_doc ? _tar.appendChild(_doc) : _tar;

          xtag.tags["json-base"].modules.push(jsn);

          }
        else if(typeOf(jsn)==="string" && k === "sheet")
          {
          this.jSheet(el,jsn);
          }
        }
  ...
  lifecycle:
    ...
    created: function()
      {
      var _data = xtag.tags['json-base'].modules.getJsonById(this.id);
      this.elements(_data);
      }
    },
...
var xBase = xtag.register( "json-base", 
    ...
    lifecycle: 
      ...
      created: function()
        ...
        var _data = base.getDomJson(this);

        // Create a new modules array else push json to modules array.
        if(xtag.tags["json-base"].modules === undefined && typeof _data.length === undefined)
          { xtag.tags["json-base"].modules = []; xtag.tags["json-base"].modules.push(_data); }
        else if(xtag.tags["json-base"].modules === undefined && typeof _data.length === "number" && typeof _data[0] === "object")
          { xtag.tags["json-base"].modules = _data; }

        var _modules = xtag.tags["json-base"].modules;

        for( var i=0; i<_modules.length; i++ )
          {
            var _xmlStg = _modules[i].is||"json-element", _parent = document.createElement(_xmlStg);
            this.jScheme(_parent, _modules[i]);
...
var xCanvas = xtag.register( "json-canvas",
  {
  mixins: ["globalMixin","schemeMixin"],
  content: "<json-menu name='canvas-menu'></json-menu>",
  methods:
    {
    },
  lifecycle:
...
var xForm = xtag.register( "json-form",
  ...
  content: function()
    {/*
    <json-switch>
      <json-switch title="Add Line"></json-switch>
      <json-switch title="Toggle"></json-switch>
      <json-switch title="Remove Line"></json-switch>
    </json-switch>
...
var xEditor =  xtag.register( "json-editor", 
    ...
    methods:
      ...
      contents: function()
        ...
        var _sheet_ = _jsn.sheet, i = null;

        if(_jsn.numbering===true){ i = 1; }
        else if(_jsn.numbering===false){ i }
