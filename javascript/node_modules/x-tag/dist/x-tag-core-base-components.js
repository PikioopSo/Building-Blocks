        ...
        for(var key in _data_)
          ...
          if(_type === "object")
            ...
            if(/style/.test(key) !== true && /sheet/.test(key) !== true)
              ...
              base.json.push(_data); base.jScheme.updateJxArray(_data);
              }
            else
              { null; }
            }
        ...
        if(_type === "object")
          ...
          if(/style/.test(key) !== true && /sheet/.test(key) !== true)
            ...
            _rt === "array" ? _data.for ? true : _data.for = "body[0]" : 
              jsn.for ? true :
                jsn.for = "#" + jsn.id;
            base.json.push(_data); _sortData(_data); 
            }
          else
            { null; }
          }
...
var schemeMixin = xtag.mixins.schemeMixin = 
  ...
  methods:
    ...
    jScheme: function(elem,_jsn)
      {
        _jsn.id ? elem.id =_jsn.id : (_jsn.id = "default" + base.jScheme.id.length, elem.id = _jsn.id),
        _jsn.class ? elem.className = _jsn.class : (_jsn.class = "default" + base.jScheme.class.length, elem.className = _jsn.class),
        _jsn.name ? elem.name = _jsn.name : (_jsn.name = "default" + base.jScheme.name.length, elem.name = _jsn.name),
        _jsn.title ? elem.setAttribute("title", _jsn.title) : (_jsn.title = "Ui Title" + base.jScheme.title.length, elem.setAttribute("title", _jsn.title))
...
var xBase = xtag.register( "json-base", 
    ...
    methods:
      ...
      updateJxArray: function(_data)
        {
        base.jScheme.updateJxArray(_data);
        return;
        },

      /* Obsolete [elements] creates new xml following json-tag rules.
      elements: function(_data)
        {
        var el = _data.for !== undefined ? _data.for._getDOM() : this;

        if(typeOf(_data) === "object" && /style/gi.test(k) !== true && k !== "sheet")
          {
      ...
      created: function()
        ...
        for( var i=0; i<_modules.length; i++ )
          ...
          if(_modules[i]["is"]==="svg")
            {doc = xtag.createFragment("<svg></svg>").childNodes[0]}
          else
          ...
          if(_modules[i]["sheet"])
            {var _doc = _modules[i]["sheet"].for ? _modules[i]["sheet"].for._getDOM() : doc; this.jSheet(_doc,_modules[i]["sheet"])}
          else
            {}
