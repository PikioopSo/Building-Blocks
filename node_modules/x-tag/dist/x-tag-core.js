...
/* [jxLock] */
  jxLock: function(options)
    ...
    vault.defs[options.keyname] ? _proto = "Key def lock already exists." : 
      ( console.log("Creating " + options.token), vault.defs[options.keyname] = new jx.jBuild(options),
  ...
  accessors: 
    ...
    dataTray:
      ...
      get: function()
        ...
        else{ return document.createElement( "auto-tray" ); }
        },
      set: function(val)
        {
        if(this.hasAttribute("data-tray") === true){ return true; }
        else{ return false + "the dataTray accessor was not set get will return auto-tray." }
        }
      },
    /* [dataItem] */
    dataItem:
      ...
      get: function()
        { 
        if(this.hasAttribute("data-item") === true && this.getAttribute("data-item") === ""){ console.log(this); return this.getAttribute("data-item")._getDOM() }
        else{ return false; }
        }
      },
    dataHidden: 
    ...
    dataActive: 
      ...
      get: function()
        {
        if(this.hetAttribute("data-active") === true) { return this.getAttribute("data-active") }
  ...
  events: 
    ...
    tap: function(event)
      ...
      if(event.target.hasAttribute("data-active") === true)
        ...
        { 
        if(event.target.parentNode.dataActive === "false")
          { _func(event.target.parentNode); event.target.parentNode.setAttribute("data-active","true"); }
          else
          { _func(event.target.parentNode); event.target.parentNode.setAttribute("data-active","false"); }
        }
      else
        {
...
/* [auto-tray] */
var autoTray = xtag.register("auto-tray", 
  {
  mixins: ["utilities"],
  lifecycle: 
    {
...
var jIco = xtag.register("j-ico", 
  ...
  events:
    ...
    tap: function(event)
      {
      var _tray = null, _selector_class = null; 

        event.target.parentNode.dataTray !== null ? 
          ( _tray = event.target.parentNode.dataTray,
            console.log("Found data-tray"),
            console.log(_tray) ): 
          ( console.log("data-tray check failed. Checking for auto-tray."),
            console.log(_tray + " was found") );

        _tray.dataSelected !== null ? 
        ( _selector_class = "." + _tray.dataSelected,
          console.log("Selector class for event from:"), console.log(this), console.log("Was assigned: " + _selector_class) ) : 
        ( console.log("data-selected for..."), 
          console.log(this), 
          console.log("Returned...  " + _selector_class + "... Attr. rquired in order to continue", 'color:skyblue') );

      if(_tray === "AUTO-TRAY"){  }
      else{
      console.log(_tray.dataTray)
      var _current = xtag.queryChildren(_tray.dataTray, _selector_class)[0], 
          _new = null;

          this.dataItem;console.log(_current)
...
window.addEventListener("scroll", function()
  ...
  if(document.body.scrollTop > 100)
    ...
    var _z = xtag.query(this.document,"json-canvas")[0];
      /curtainsDown\-1/.test(_z.className) !== true ? "" : (_z.className = "_"); 

    var _p = xtag.query(this.document,"#header-toggle-btn")[0];
